...
const ratingService = {
    ...
    getRatingStats: async (targetType, targetId) => {
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/ratings/summary/${targetType === 'sti_service' || targetType === 'sti-service' ? 'sti-service' : 'consultant'}/${targetId}?includeRecentReviews=true`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    getUserRating: async (targetType, targetId) => {
        try {
            ...
            const mappedTargetType = targetType.toLowerCase() === 'sti_service' || targetType.toLowerCase() === 'sti-service'
                ? 'STI_SERVICE'
                : 'CONSULTANT';
